...
#include <esp_wifi.h>  // Biblioteca para diagnósticos avançados;
...
const char* ssid = "Teste"; //Nome da rede Wi-fi internet de casa MEO-FODF40_EXT que é 2.4 GHz porque o ESP32 não suporta redes 5 GHz;
const char* password = "012345678"; // Senha da rede Wi-Fi;

//Servidor configuração:
const char* serverIP ="192.168.255.5";//IP do servidorTCP;
const int serverPort = 8090;//Porta do servidor TCP;
const char* serverURL ="image.png";//URL da API HTTP;
...
void connectToWiFi(){
  ...
  // Obtém informações detalhadas sobre a conexão
    ...
    if (esp_wifi_sta_get_ap_info(&ap_info) == ESP_OK) {
      Serial.println("Informações da AP recebidas.");
    } else {
      Serial.println("Erro ao obter informações da AP.");
    }

    // Verifica o status e mostra a razão da falha
    switch (WiFi.status()) {
      case WL_NO_SSID_AVAIL:
        Serial.println("SSID não disponível.");
        break;
      case WL_CONNECT_FAILED:
        Serial.println("Falha na conexão. Verifique a senha.");
        break;
      case WL_IDLE_STATUS:
        Serial.println("Estado ocioso.");
        break;
...
void sendTrafficData(void* arg) {
  if (WiFi.status() == WL_CONNECTED) {  // Verificar se está conectado ao Wi-Fi
    ...
    // Criar payload JSON com o estado atual
    ...
    if (httpResponseCode > 0) {
      ...
      if (response.indexOf("\"NEXT\"") >= 0) { // Verificar se o estado "NEXT" foi enviado
        changeTrafficLight(nullptr); // Alterar para o próximo estado
        Serial.println("Estado alterado para o próximo (NEXT).");
      }
    } else {
